const CONSTANTS = {
  ...
  calculateDyePerTap: function(tier, damage, activeBoosters, color) {
    ...
    if (player.skills.bonus_color_drop) {
      baseDye *= SKILLS.find(function(s) { return s.id === 'bonus_color_drop'; }).getEffect(player.skills.bonus_color_drop);
    }
    if (activeBoosters.some(function(b) { return b.key === 'doubleDye60s'; })) {
      baseDye *= 2;
    }
    ...
    for (var dyeColor in dyeMix) {
      if (dyeMix.hasOwnProperty(dyeColor)) {
        result[dyeColor] = Math.ceil(baseDye * dyeMix[dyeColor]);
      }
    }
    return result;
  },
  ...
  calculateKillBonus: function(tier, activeBoosters, color) {
    ...
    if (player.skills.bonus_color_drop) {
      bonus *= SKILLS.find(function(s) { return s.id === 'bonus_color_drop'; }).getEffect(player.skills.bonus_color_drop);
    }
    if (activeBoosters.some(function(b) { return b.key === 'doubleDye60s'; })) {
      bonus *= 2;
    }
